<?php
    ...
    $content = '';
    $gmb_config = 'gmb_config:1111111111';

    $gm = rex_sql::factory();
    $gm->debugsql = 0;
    $gm->setQuery('select * from rex_module where output LIKE "%' . $gmb_config . '%"');
    ...
    $content .= '-- '.$gmb_config.'<p>'.$this->i18n('install_modul_description').'<br /><br />';

    if ($module_id > 0) {
        $content .= '444<p><a class="btn btn-primary" href="index.php?page=grid_modul_builder/install&amp;install=1&amp;module_id=' . $module_id . '" class="rex-button">' . $this->i18n('grid_modul_builder_install_update_modul', htmlspecialchars($module_name)) . '</a></p>';

    }else {
        $content .= '3333<p><a class="btn btn-primary" href="index.php?page=grid_modul_builder/install&amp;install=1" class="rex-button">' . $this->i18n('grid_modul_builder_install_modul', $grid_modul_builder_module_name) . '</a></p>';
